...
public class HUD
    ...
    public void onRender2D(final Render2DEvent event) {
        ...
        switch (this.watermark.getValue()) {
            case PHOBOS: {
                this.renderer.drawString("Nsm Phobos" + (this.modeVer.getValue() ? " v2.0.1" : ""), 2.0f, 2.0f, (this.rolling.getValue() && this.rainbow.getValue()) ? this.colorMap.get(2) : this.color, true);
                break;
            }
            case EARTH: {
                this.renderer.drawString("3arthh4ck" + (this.modeVer.getValue() ? " v2.0.1" : ""), 2.0f, 2.0f, (this.rolling.getValue() && this.rainbow.getValue()) ? this.colorMap.get(2) : this.color, true);
                break;
            }
            case CUSTOM: {
                this.renderer.drawString(this.customWatermark.getValue() + (this.modeVer.getValue() ? " v2.0.1" : ""), 2.0f, 2.0f, (this.rolling.getValue() && this.rainbow.getValue()) ? this.colorMap.get(2) : this.color, true);
                break;
            }
        }
        ...
        if (this.renderingUp.getValue()) {
            if (this.serverBrand.getValue()) {
                final String text2 = grayString + "Server brand " + Phobos.serverManager.getServerBrand();
            ...
            if (this.speed.getValue()) {
                final String text2 = grayString + "Speed " + Phobos.speedManager.getSpeedKpH() + " km/h";
            ...
            if (this.time.getValue()) {
                final String text2 = grayString + "Time " + new SimpleDateFormat("h:mm a").format(new Date());
            ...
            if (this.durability.getValue()) {
                ...
                if (itemDamage > 0) {
                    final String text = grayString + "Durability " + itemDamage;
            ...
            if (this.tps.getValue()) {
                final String text2 = grayString + "TPS " + Phobos.serverManager.getTPS();
            ...
            final String fpsText = grayString + "FPS " + Minecraft.debugFPS;
            final String text = grayString + "Ping " + (ServerModule.getInstance().isConnected() ? ServerModule.getInstance().getServerPing() : Phobos.serverManager.getPing()) + (this.MS.getValue() ? "ms" : "");
            ...
            final String fpsText = grayString + "FPS " + Minecraft.debugFPS;
            final String text = grayString + "Ping " + Phobos.serverManager.getPing();
            if (this.renderer.getStringWidth(text) > this.renderer.getStringWidth(fpsText)) {
                if (this.ping.getValue()) {
                    this.renderer.drawString(text, (float) (width - (this.renderer.getStringWidth(text) + 2)), (float) (2 + k++ * 10), (this.rolling.getValue() && this.rainbow.getValue()) ? this.colorMap.get(2 + k * 10) : this.color, true);
                }
    ...
    public void renderGreeter() {
        ...
        switch (this.greeter.getValue()) {
            ...
            case LONG: {
                text = text + "Welcome to Nsm Phobos " + Phobos.MODVER + " " + HUD.mc.player.getDisplayNameString() + " :^)";
                break;
            }
            case CUSTOM: {
