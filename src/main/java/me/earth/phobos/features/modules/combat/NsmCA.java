...
public class NsmCA extends Module {
    ...
    public NsmCA() {
        super("NsmCA", "cool ca", Module.Category.COMBAT, true, false, false);
    }
    ...
    public void onCrystal() {
        ...
        if (this.hotBarSlot != -1 && this.autoswitch.getValue() && !AutoCrystal.mc.player.isPotionActive(MobEffects.WEAKNESS) && this.autoswitch.getValue() == true && !this.silentswitch.getValue()) {
        ...
        EnumHand hand = null;
        if (slot != -1 && this.silentswitch.getValue()) {
            if (mc.player.isHandActive()) {
                hand = mc.player.getActiveHand();
            }
            mc.player.connection.sendPacket(new CPacketHeldItemChange(slot));
        } else if (this.hotBarSlot != -1 && this.autoswitch.getValue().booleanValue() && !AutoCrystal.mc.player.isPotionActive(MobEffects.WEAKNESS) && this.silentswitch.getValue().booleanValue() == false) {
            AutoCrystal.mc.player.inventory.currentItem = this.hotBarSlot;
        }
    ...
    public void onRender3D(Render3DEvent event) {
        if (this.pos != null && this.render.getValue().booleanValue() && this.target != null) {
            RenderUtil.drawBoxESP(this.pos, ClickGui.getInstance().rainbow.getValue() != false ? ColorUtil1.rainbow(ClickGui.getInstance().rainbowHue.getValue()) : new Color(this.red.getValue(), this.green.getValue(), this.blue.getValue(), this.alpha.getValue()), this.outline.getValue(), ClickGui.getInstance().rainbow.getValue() != false ? ColorUtil1.rainbow(ClickGui.getInstance().rainbowHue.getValue()) : new Color(this.cRed.getValue(), this.cGreen.getValue(), this.cBlue.getValue(), this.cAlpha.getValue()), this.lineWidth.getValue().floatValue(), this.outline.getValue(), this.box.getValue(), this.boxAlpha.getValue(), true);
    ...
    EntityPlayer getTarget() {
        ...
        for (EntityPlayer entity : AutoCrystal.mc.world.playerEntities) {
            if (AutoCrystal.mc.player == null || AutoCrystal.mc.player.isDead || entity.isDead || entity == AutoCrystal.mc.player || Phobos.friendManager.isFriend(entity.getName()) || entity.getDistance(AutoCrystal.mc.player) > 12.0f)
                continue;
            this.armorTarget = false;
    ...
    private NonNullList<BlockPos> placePostions(float placeRange) {
        ...
        positions.addAll(NsmCA.getSphere(new BlockPos(Math.floor(AutoCrystal.mc.player.posX), Math.floor(AutoCrystal.mc.player.posY), Math.floor(AutoCrystal.mc.player.posZ)), placeRange, (int) placeRange, false, true, 0).stream().filter(pos -> this.canPlaceCrystal(pos, true)).collect(Collectors.toList()));
        return positions;
    }
